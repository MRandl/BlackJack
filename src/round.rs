...
mod test {
    ...
    #[test] 
    fn play_round_test() {
        let card1 = Card{rank : Rank::Two, suit: Suit::Diamonds};
        let card2 = Card{rank : Rank::Two, suit: Suit::Clubs};

        play_round(&mut vec!((vec!(card1), None), (vec!(card2), None)), &mut vec!(), &mut vec!(), &vec!(Player::Bot, Player::Bot, Player::Bot), &mut vec!(0,0,0), &mut vec!(0,0,0))
    }
}
